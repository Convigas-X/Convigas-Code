...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          bolt_user_profile: this._safeGetItem('convigas_code_user_profile'),
          bolt_settings: this._safeGetItem('convigas_code_settings'),
          bolt_profile: this._safeGetItem('convigas_code_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('convigas_code_viewed_features'),
          developer_mode: this._safeGetItem('convigas_code_developer_mode'),
        ...
        ui: {
          // Tab configuration
          bolt_tab_configuration: this._safeGetItem('convigas_code_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('convigas_code_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('convigas_code_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          bolt_read_logs: this._safeGetItem('convigas_code_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('convigas_code_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('convigas_code_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.bolt_tab_configuration) {
        try {
          this._safeSetItem('convigas_code_tab_configuration', data.ui.bolt_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'convigas_code_acknowledged_debug_issues',
        'convigas_code_acknowledged_connection_issue',
        'error_logs',
        'convigas_code_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('convigas_code_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
